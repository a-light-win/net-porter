...
pub fn getUid(username: []const u8) ?std.posix.uid_t {
    ...
    const ret = c.getpwnam_r(&username[0], &pwd, &buf[0], buf.len, &result);
...
pub fn getGid(groupname: []const u8) ?std.posix.gid_t {
