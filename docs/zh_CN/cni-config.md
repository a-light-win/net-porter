...
net-porter 支持两种 CNI 网络配置方式：

| 方式 | 配置位置 | 适用场景 |
|------|---------|---------|
| **标准 CNI 配置（推荐）** | `/etc/net-porter/cni.d/` | 支持标准 CNI 1.0 格式，包括链式插件 |
| **内联资源配置（旧版）** | `/etc/net-porter/config.json` 的 `resources` 字段 | 简单场景的向后兼容 |
...
如果你在 `config.json` 中定义了 `resources` 字段，net-porter 会同时加载 `cni.d/` 目录和 `resources` 中的配置。如果两者定义了相同名称的网络，`cni.d/` 中的配置优先。
...
> **建议**：对于新部署，使用 `cni.d/` 目录方式。`resources` 字段将在未来版本中移除。
