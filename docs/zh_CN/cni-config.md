...
> **重要**：
> - `address` 字段**必须**使用 CIDR 格式（如 `192.168.1.0/24`），不能省略前缀长度。
> - net-porter 会从模板地址中提取前缀（如 `/24`），与用户请求的 IP 组合成最终容器地址（如 `192.168.1.15/24`）。
> - 如果省略前缀（如写成 `"192.168.1.0"`），静态 IP 注入将静默跳过，容器无法获得 IP 地址。
