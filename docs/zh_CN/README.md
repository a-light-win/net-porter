...
│                                                              │
│  Server                                                      │
│  ├── AclScanner: 扫描 acl.d/ → 将用户名解析为 UID            │
...
│  │  2. 加载 ACL 授权 + 通过 inotify 热重载                  │  │
│  │  3. 接受连接 → 派生处理线程                              │  │
│  │  4. 通过 /proc/<catatonit_pid>/root/ 解析 netns 路径     │  │
│  │  5. 每次请求验证 catatonit 存活 + nsfs 设备检查          │  │
│  │  6. 在宿主命名空间中执行 CNI 插件                        │  │
│  └─────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
```
...
- **Per-UID Worker 进程**：每个用户拥有独立的 worker 进程（crash 隔离、运行在宿主命名空间）
...
│   │   └── AclScanner.zig       # 服务端 ACL 扫描器（用户名 → UID 解析）
...
│   ├── cni/
│   │   ├── Cni.zig               # CNI 执行逻辑
│   │   ├── CniConfig.zig         # CNI 配置管理
│   │   ├── CniLoader.zig         # CNI 配置文件加载 & 校验
│   │   ├── PluginConf.zig        # 插件配置辅助工具
│   │   ├── Attachment.zig        # CNI attachment 状态管理
...
│   ├── nfpm/                     # nfpm 打包配置
│   └── cni-example.conflist      # 示例 CNI 配置
├── build.zig                     # Zig 构建配置
└── justfile                      # Just 任务定义
```
...
1.1.0 是一个向后兼容的安全和稳定性版本，无需修改配置即可升级。
...
旧名称仍可使用，但会打印废弃警告。建议在方便时更新 podman 网络选项：
...
- 确保配置文件中的 `name` 字段与你通过 `resource` 传入的值匹配
...
4. **Netns 路径解析**：CNI_NETNS 通过 `/proc/<catatonit_pid>/root/<path>` 解析，穿透进入 catatonit 的挂载命名空间。每次请求验证 catatonit 仍然存活（UID + 进程名检查），并验证解析路径为 nsfs 文件（设备 0:4）
